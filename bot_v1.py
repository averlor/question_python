...
def main():
    ...
    keywords = ['crm', 'црм', 'продукт', 'обучение', 'битрикс', 'корпоративный портал', 'внедрение crm', 'интеграция', 'телефония', 'подключить',
                'подключение', 'настройка', 'тариф', 'платный', 'консультации', 'консультация', 'управление', 'почта', '﻿соц. сети', '﻿отдел продаж', 'лид',
                'сделка', '﻿база клиентов', '﻿базы клиентов', '﻿бизнес-процессы', 'роботы', 'триггеры', 'роботов',
                'триггеров', 'перенос', 'переход', 'аналитика', 'отчет']
    request_arr = [
    ...
    for request in request_arr:
        for word in keywords:
            if (word in request['description'].lower()) and (request not in result):
                result.append(request)                          # неверно работает
                #element.click()
